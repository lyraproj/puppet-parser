...
func init() {
	...
	HardIssue(LEX_HEREDOC_ILLEGAL_ESCAPE, `illegal heredoc escape '%{flag}'`)
	...
	HardIssue(LEX_INVALID_OPERATOR, `invalid operator '%{op}'`)
	...
	HardIssue(LEX_UNEXPECTED_TOKEN, `unexpected token '%{token}'`)
	HardIssue(LEX_UNTERMINATED_COMMENT, `unterminated /* */ comment`)
	HardIssue(LEX_UNTERMINATED_STRING, `unterminated %{string_type} quoted string`)
	...
	HardIssue(PARSE_RESOURCE_WITHOUT_TITLE, `This expression is invalid. Did you try declaring a '%{name}' resource without a title?`)
