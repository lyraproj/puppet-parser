...
func init() {
  ...
  HardIssue(LEX_HEREDOC_ILLEGAL_ESCAPE, `illegal heredoc escape '%s'`)
  ...
  HardIssue(LEX_INVALID_OPERATOR, `invalid operator '%s'`)
  ...
  HardIssue(LEX_UNEXPECTED_TOKEN, `unexpected token '%s'`)
  HardIssue(LEX_UNTERMINATED_COMMENT, `unterminated /* */ comment`)
  HardIssue(LEX_UNTERMINATED_STRING, `unterminated %s quoted string`)
  ...
  HardIssue(PARSE_RESOURCE_WITHOUT_TITLE, `This expression is invalid. Did you try declaring a '%s' resource without a title?`)
